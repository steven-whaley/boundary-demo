Markdown Live PreviewResetCopy

- Multi-hop using an HCP ingress workers and a private egress worker
...
Features
SSH Credential Injection with username templating
RDP target with AD credentials brokered from the Vault LDAP secrets engine
Okta integration using managed groups to show how different users get different sets of targets based on OIDC group membership
Session recording
K8s target
Database target with brokered credentials from the Vault DB secrets engine
Multi-hop using an HCP ingress workers and a private egress worker
...
Connect as admin user. The admin_auth_method_id can be found in the outputs of the boundary-demo-init workspace: boundary authenticate password -auth-method-id <admin_auth_method_id>

Connect as Okta user: boundary authenticate
